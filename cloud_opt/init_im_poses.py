...
def fast_pnp(pts3d, focal, msk, device, pp=None, niter_PnP=10):
    ...
    for focal in tentative_focals:
        ...
        # 1. 明确获取 PnP 的输入
        obj_points = pts3d[msk]
        img_points = pixels[msk]

        # 2. 定义 PnP 所需的最小点数
        # PnP 在数学上至少需要 4 个点，为了 RANSAC 的鲁棒性，通常需要更多
        MIN_PNP_POINTS = 4  # 你可以根据需要将其调高到 6 或 8

        # 3. 检查点的数量
        if obj_points.shape[0] < MIN_PNP_POINTS:

            print(f"警告: PnP (sqpnp) 跳过，因为点太少。"
                f"(Got {obj_points.shape[0]}, 需求 >= {MIN_PNP_POINTS})")
            
            # 返回一个 "失败" 结果，而不是让 OpenCV 崩溃
            success = False
            R, T, inliers = None, None, None
        
        else:
            # 4. 只有在有足够点的情况下才运行 PnP
            try:
                success, R, T, inliers = cv2.solvePnPRansac(obj_points, img_points, K, None,
                                                    iterationsCount=niter_PnP, reprojectionError=5, flags=cv2.SOLVEPNP_SQPNP)
            except cv2.error as e:
                # 捕获其他可能的 OpenCV 错误 (例如 K 矩阵无效)
                print(f"警告: cv2.solvePnPRansac 即使有足够的点也失败了: {e}")
                success = False
                R, T, inliers = None, None, None
